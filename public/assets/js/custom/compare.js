function showTab(car) {
    ...
    if (car === "car1") {
        ...
        document
            .getElementById("tabCar1")
            .classList.add("custom-underline", "text-black", "border-blue-500");
...
function getSelectedCars() {
    ...
    checkboxes.forEach((checkbox) => {
        ...
        const trunk = checkbox.getAttribute("trunk")
            ? parseInt(
                  checkbox.getAttribute("trunk").replace(" cubic feet", "")
              )
            : 0; // Xử lý khi null
    ...
    checkboxes.forEach((checkbox) => {
        ...
        if (checkbox.checked) {
            ...
            const accelerationTime =
                parseFloat(checkbox.getAttribute("acceleration_time")) || 0;
            const fuelEfficiency =
                parseFloat(checkbox.getAttribute("fuel_efficiency")) || 0;
...
`;
    ...
    if (selectedCars.length === 2) {
        ...
        contentCar1.innerHTML = `
          <h2 class="text-2xl font-bold">Why is ${
              selectedCars[0].name
          } better than ${selectedCars[1].name}</h2>
          ...
          ${
            ...
            <li class="flex items-center mb-2">
                ...
                <div class="ml-2">
                    <p><b>${Math.abs(
                        diffPrice
                    )} dollar</b> cheaper, making it a more budget-friendly option</p>
                    <small>${selectedCars[0].price} vs ${
      ...
      <h2 class="text-2xl font-bold">Why is ${
          selectedCars[1].name
      } better than ${selectedCars[0].name}</h2>
      <ul class="list-none mt-4">
       ...
       ${
          ...
          <li class="flex items-center mb-2">
              <span class="text-green-600 font-bold">✔</span>
              <div class="ml-2">
                  <p><b>${Math.abs(diffSeat).toFixed(
                      2
                  )} seats</b> higher seating capacity</p>
                  <small>${selectedCars[1].seat} vs ${
                        selectedCars[0].seat
                    }</small>
              </div>
          </li>`
          ...
          <li class="flex items-center mb-2">
              <span class="text-green-600 font-bold">✔</span>
              <div class="ml-2">
                  <p><b>${Math.abs(diffPower).toFixed(
                      2
                  )} HP</b> more engine power</p>
                  <small>${selectedCars[1].power} HP vs ${
                        selectedCars[0].power
                    } HP</small>
              </div>
          </li>`
                  : ""
          ...
          <li class="flex items-center mb-2">
              <span class="text-green-600 font-bold">✔</span>
              <div class="ml-2">
                  <p><b>${Math.abs(diffSpeed).toFixed(
                      2
                  )} km/h</b> faster max speed</p>
                  <small>${selectedCars[1].speed} km/h vs ${
                        selectedCars[0].speed
                    } km/h</small>
              </div>
          </li>`
                  : ""
          ...
          <li class="flex items-center mb-2">
              <span class="text-green-600 font-bold">✔</span>
              <div class="ml-2">
                  <p><b>${Math.abs(diffTrunk).toFixed(
                      2
                  )} cubic feet</b> larger trunk capacity</p>
                  <small>${selectedCars[1].trunk} vs ${
                        selectedCars[0].trunk
                    }</small>
              </div>
          </li>`
                  : ""
    ...
    const car1LengthPercent = (
        (selectedCars[0].length / totalLength) *
        100
    ).toFixed(2);
    const car2LengthPercent = (
        (selectedCars[1].length / totalLength) *
        100
    ).toFixed(2);
    ...
    document.getElementById(
        "car1-length-value"
    ).textContent = `${selectedCars[0].length} mm`;
    document.getElementById(
        "car1-length-bar"
    ).style.width = `${car1LengthPercent}%`;

    // Cập nhật giá trị và thanh tiến trình cho xe 2
    document.getElementById(
        "car2-length-value"
    ).textContent = `${selectedCars[1].length} mm`;
    document.getElementById(
        "car2-length-bar"
    ).style.width = `${car2LengthPercent}%`;

    const totalWidth = selectedCars[0].width + selectedCars[1].width;
    const car1WidthPercent = (
        (selectedCars[0].width / totalWidth) *
        100
    ).toFixed(2);
    const car2WidthPercent = (
        (selectedCars[1].width / totalWidth) *
        100
    ).toFixed(2);
    ...
    document.getElementById(
        "car1-width-value"
    ).textContent = `${selectedCars[0].width} mm`;
    document.getElementById(
        "car1-width-bar"
    ).style.width = `${car1WidthPercent}%`;

    // Cập nhật giá trị và thanh tiến trình cho xe 2
    document.getElementById(
        "car2-width-value"
    ).textContent = `${selectedCars[1].width} mm`;
    document.getElementById(
        "car2-width-bar"
    ).style.width = `${car2WidthPercent}%`;

    const totalHeight = selectedCars[0].height + selectedCars[1].height;
    const car1HeightPercent = (
        (selectedCars[0].height / totalHeight) *
        100
    ).toFixed(2);
    const car2HeightPercent = (
        (selectedCars[1].height / totalHeight) *
        100
    ).toFixed(2);
    ...
    document.getElementById(
        "car1-height-value"
    ).textContent = `${selectedCars[0].height} mm`;
    document.getElementById(
        "car1-height-bar"
    ).style.width = `${car1HeightPercent}%`;

    // Cập nhật giá trị và thanh tiến trình cho xe 2
    document.getElementById(
        "car2-height-value"
    ).textContent = `${selectedCars[1].height} mm`;
    document.getElementById(
        "car2-height-bar"
    ).style.width = `${car2HeightPercent}%`;

    //<!-- Performance Cards ---------------------------------------------------->
    // Tính tổng và phần trăm cho từng thuộc tính mới
    const totalAccelerationTime =
        selectedCars[0].acceleration_time + selectedCars[1].acceleration_time;
    const car1AccelerationPercent = (
        (selectedCars[0].acceleration_time / totalAccelerationTime) *
        100
    ).toFixed(2);
    const car2AccelerationPercent = (
        (selectedCars[1].acceleration_time / totalAccelerationTime) *
        100
    ).toFixed(2);

    const totalFuelEfficiency =
        selectedCars[0].fuel_efficiency + selectedCars[1].fuel_efficiency;
    const car1EfficiencyPercent = (
        (selectedCars[0].fuel_efficiency / totalFuelEfficiency) *
        100
    ).toFixed(2);
    const car2EfficiencyPercent = (
        (selectedCars[1].fuel_efficiency / totalFuelEfficiency) *
        100
    ).toFixed(2);

    const totalTorque = selectedCars[0].torque + selectedCars[1].torque;
    const car1TorquePercent = (
        (selectedCars[0].torque / totalTorque) *
        100
    ).toFixed(2);
    const car2TorquePercent = (
        (selectedCars[1].torque / totalTorque) *
        100
    ).toFixed(2);
    ...
    // Acceleration
    document.getElementById(
        "car1-acceleration-value"
    ).textContent = `${selectedCars[0].acceleration_time} s`;
    document.getElementById(
        "car1-acceleration-bar"
    ).style.width = `${car1AccelerationPercent}%`;

    document.getElementById(
        "car2-acceleration-value"
    ).textContent = `${selectedCars[1].acceleration_time} s`;
    document.getElementById(
        "car2-acceleration-bar"
    ).style.width = `${car2AccelerationPercent}%`;

    // Efficiency
    document.getElementById(
        "car1-efficiency-value"
    ).textContent = `${selectedCars[0].fuel_efficiency} km/l`;
    document.getElementById(
        "car1-efficiency-bar"
    ).style.width = `${car1EfficiencyPercent}%`;

    document.getElementById(
        "car2-efficiency-value"
    ).textContent = `${selectedCars[1].fuel_efficiency} km/l`;
    document.getElementById(
        "car2-efficiency-bar"
    ).style.width = `${car2EfficiencyPercent}%`;

    // Torque
    document.getElementById(
        "car1-torque-value"
    ).textContent = `${selectedCars[0].torque} Nm`;
    document.getElementById(
        "car1-torque-bar"
    ).style.width = `${car1TorquePercent}%`;

    document.getElementById(
        "car2-torque-value"
    ).textContent = `${selectedCars[1].torque} Nm`;
    document.getElementById(
        "car2-torque-bar"
    ).style.width = `${car2TorquePercent}%`;

    //-------------------------------------------------------------------------
    const legendItems = document.querySelectorAll(
        ".radar-legend .legend-item span"
    );
    if (selectedCars[0]) {
    ...
    const chartData = {
        ...
        car1: selectedCars[0]
            ? [
                  selectedCars[0].speedp,
                  selectedCars[0].seatp,
                  selectedCars[0].powerp,
                  selectedCars[0].trunkp,
                  selectedCars[0].pricep,
              ]
            : [0, 0, 0, 0, 0],
        car2: selectedCars[1]
            ? [
                  selectedCars[1].speedp,
                  selectedCars[1].seatp,
                  selectedCars[1].powerp,
                  selectedCars[1].trunkp,
                  selectedCars[1].pricep,
              ]
            : [0, 0, 0, 0, 0],
    };
...
class RadarChart {
    ...
    drawAxis() {
        this.data.labels.forEach((label, i) => {
            const angle =
                (Math.PI * 2 * i) / this.data.labels.length - Math.PI / 2;
    ...
    drawData() {
        ...
        path1.setAttribute("fill", "rgba(100, 180, 255, 0.5)");
        path1.setAttribute("stroke", "#64b4ff");
        ...
        path2.setAttribute("fill", "rgba(255, 150, 180, 0.5)");
        path2.setAttribute("stroke", "#ff96b4");
